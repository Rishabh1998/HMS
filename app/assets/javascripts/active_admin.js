...
$( document ).ready(function() {
    ...
    if (key == "status" && value == "checkout"){
        
        $('select').on('change', function(){
            checkout_submit = document.getElementById('booking_submit_action');
            ...
            for (let i = 0; i < selects.length; i++) { 
                if (selects[i].childNodes[1].textContent == "Unpaid"){
                    ...
                    selects[i].parentElement.style.border = "";
            }

            if (!unpaid)
                checkout_submit.childNodes[0].setAttribute("disabled", "");
            else 
                checkout_submit.childNodes[0].removeAttribute("disabled", "");   
        });
        $('select').trigger('change');
    }

    if (key == "checkin_receipt_id" || key == "checkout_receipt_id"){

        printed = document.getElementById('checkin_receipt_printed').childNodes[3].childNodes[1].textContent
        endpoint = "check_in_printed";
        if (key == "checkout_receipt_id"){
            printed = document.getElementById('checkout_receipt_printed').childNodes[3].childNodes[1].textContent
            endpoint = "check_out_printed"
        }
        if (printed == "No"){
            $.ajax({
                    type: "GET",
                    url: "/admin/bookings/" + value + "/" + endpoint,
                    success: function(data) {
                    }
            });

            pdfWindow = window.open("/pdfs/filename.pdf");
            pdfWindow.focus();
            // pdfWindow.print();
            pdfWindow.print();
        }
    ...
    $( "#washroom_image" ).change(function() {
        ...
        room = document.getElementById('room_image').value;
        laundry = document.getElementById('laundry').value;

        if (washroom != "" && room != "" && laundry != "")
            update_room.childNodes[0].removeAttribute("disabled");
        else
            update_room.childNodes[0].setAttribute("disabled", "");
    });
